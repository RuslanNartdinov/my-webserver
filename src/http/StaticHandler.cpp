...
namespace ws
{
                                           ...
                                           const std::string &reqPath, bool &ok)
    {
        ...
        if (loc)
        {
            ...
            // Если URL начинается с location.path — отрезаем этот префикс
            if (rest.size() >= lpath.size() && rest.compare(0, lpath.size(), lpath) == 0)
            {
                std::string tail = rest.substr(lpath.size());
                if (!tail.empty() && tail[0] == '/')
                    tail.erase(0, 1);

                if (!loc->alias.empty())      base = loc->alias;
                else if (!loc->root.empty())  base = loc->root;
                else                          base = srv.root.empty() ? "." : srv.root;
